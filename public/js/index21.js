...
function doSelect(){
	...
	if(area==1){
			...
			location.href = mainArray[topPos].showArr[menuPos].listArray[0].url;
	}
	else if(area==3){
		if(btnPos==0)pageUp();
...
function getNodeContent(){
	...
	var token = getGlobalVar('tokenMsg'); 
	var bind_regionid = getGlobalVar('bind_regionid'); 
...
function getNodeContentListCb(ret){
    ...
    var type = mainArrayNew[menuPos]["display_style"];
    if(type==0){
    	showList();
    }else{
    	showList1();
...
function menuFocus(__num){
	...
	var tempPos = menuPos;
	menuPos+=__num;	
	var len = mainArray[topPos].showArr.length;
...
function showList(){
	...
	for(i=0; i<listSize; i++){
		if(position + i<mainArray[topPos].showArr[menuPos].listArray.length){
			//$("list_title").innerHTML = mainArray[topPos].showArr[menuPos].menuTitle.slice(0,wordSize1);	
			$("title0_"+i).innerHTML=mainArray[topPos].showArr[menuPos].listArray[i].title.slice(0,wordSize_1);
...
function showList1(){
	...
	for(i=0; i<listSize; i++){
		if(position+i<mainArray[topPos]["showArr"][menuPos].listArray.length){
			...
			$("title1_"+i).innerHTML = mainArray[topPos]["showArr"][menuPos].listArray[position+i].title.slice(0,wordSize2);
			$("img1_"+i).src = mainArray[topPos].showArr[menuPos].listArray[position+i].img;
		}
		else {
...
function listFocus(__num){
	...
	if(type == 0){
		$("title0_"+listPos%listSize).innerHTML = "　◎　"+mainArray[topPos].showArr[menuPos].listArray[listPos].title.slice(0,wordSize_1);
		...
		$("title1_"+listPos%listSize).innerHTML = mainArray[topPos].showArr[menuPos].listArray[listPos].title.slice(0,wordSize2);
	...
	if(type == 0){
		...
		if(mainArray[topPos].showArr[menuPos].listArray[listPos].title.length>wordSize){
				$("title0_"+listPos%listSize).innerHTML = '<marquee direction="left" width="98%" scrollamonut="3" scrolldelay="100"">'+ mainArray[topPos].showArr[menuPos].listArray[listPos].title +'</marquee>';
		}
	}else{

		//$("list1_"+listPos%listSize).style.backgroundImage = "";
		$("pic1_"+listPos%listSize).style.background= "url(../public/image/listPicSecFocus1.png)";
		if(mainArray[topPos].showArr[menuPos].listArray[listPos].title.length>wordSize2){
			$("title1_"+listPos%listSize).innerHTML = '<marquee direction="left" width="99%" scrollamonut="3" scrolldelay="100"">'+ mainArray[topPos].showArr[menuPos].listArray[listPos].title +'</marquee>';
		}
	}
	//posArray[menuPos] = listPos;
}
	...
	switch(key_code){
		case "KEY_UP":
			if(area==0){
				...
				if(menuPos==0){
					area = 0 ;
				}else if(menuPos==mainArray[topPos].showArr[menuPos].listArray.length-1){
					menuFocus(0);
					$("menuSecond"+menuPos%menuSize).style.background = "#F6EEA8";
				}else{
					menuFocus(-1);
